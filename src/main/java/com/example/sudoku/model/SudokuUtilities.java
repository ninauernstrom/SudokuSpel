...
public class SudokuUtilities {
    ...
    static int[][][] convertStringToIntMatrix(String stringRepresentation) {
        ...
        values = randomizeMatrix(values);

        return values;
    }

    private static int[][][] randomizeMatrix(int[][][] values){
    ...
    private static int[][][] randomizeNumbers(int[][][] values){
        Random rand = new Random();
        ...
        for(int row = 0; row < GRID_SIZE; row++){
            for (int col = 0; col < GRID_SIZE; col++){
                if(values[row][col][0] == numberOne){
                    values[row][col][0] = numberTwo;
                } else if(values[row][col][0] == numberTwo){
                    values[row][col][0] = numberOne;
                }
                if(values[row][col][1] == numberOne){
                    values[row][col][1] = numberTwo;
                } else if(values[row][col][1] == numberTwo){
                    values[row][col][1] = numberOne;
                }
            }
        }
        return values;
    ...
    private static int[][][] mirrorHorizontal(int values[][][]){
        int[][][] temp = new int[GRID_SIZE][GRID_SIZE][2];
        for(int row = 0; row < GRID_SIZE; row++){
            int j = GRID_SIZE;
            for(int col = 0; col < GRID_SIZE; col++){
                temp[row][col] = values[row][j-1];
                j--;
            }
        }
        return temp;
    ...
    private static int[][][] mirrorVertical(int values[][][]){
        int[][][] temp = new int[GRID_SIZE][GRID_SIZE][2];
        int j = GRID_SIZE;
        for(int row = 0; row < GRID_SIZE; row++){
            temp[row] = values[j-1];
